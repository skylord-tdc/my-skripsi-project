...
if (isset($_POST['tombol'])) {
    ...
    $cekdulu = "select * from data_pengumuman where judul='$_POST[judul]'";
    $prosescek = mysqli_query($conn, $cekdulu);
    if (mysqli_num_rows($prosescek) > 0) { //proses mengingatkan data sudah ada
        // alert boostrap
        // session_start();
        $_SESSION["warning"] = '<div class="alert alert-warning" role="alert">Pengumuman sudah tersedia</div>';
        //   end alert
        ...
        $folder = "logic/pengumuman_sklh/";
        move_uploaded_file($temp, $folder . $name);
        mysqli_query($conn, "insert into data_pengumuman (slug,judul,isi,tanggal,foto,tipe_gambar,ukuran_gambar)
        values (
            '$slug',
            '$judul',
            '$isi',
        ...
        // alert boostrap
        // session_start();
        $_SESSION["success"] = '<div class="alert alert-success" role="alert">Pengumuman berhasil ditambahkan</div>';
        //   end alert
